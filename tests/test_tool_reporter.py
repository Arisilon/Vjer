...
from vjer.tool_reporter import (tool_reporter, get_version, get_helm_info,
                                PRODUCTS)


class TestToolReporter(unittest.TestCase):
    """Tests for the tool_reporter function."""

    @patch('vjer.tool_reporter.get_helm_info')
    @patch('vjer.tool_reporter.get_version')
    def test_tool_reporter(self, mock_get_version: MagicMock,
                          mock_get_helm_info: MagicMock) -> None:
        """Test tool_reporter function."""
        mock_get_version.return_value = '1.0.0'
        mock_get_helm_info.side_effect = [
    ...
    @patch('vjer.tool_reporter.get_helm_info')
    @patch('vjer.tool_reporter.get_version')
    def test_tool_reporter_calls_get_version(self, mock_get_version: MagicMock,
                                            mock_get_helm_info: MagicMock) -> None:
    ...
    @patch('vjer.tool_reporter.get_helm_info')
    @patch('vjer.tool_reporter.get_version')
    def test_tool_reporter_calls_get_helm_info(self, mock_get_version: MagicMock,
                                              mock_get_helm_info: MagicMock) -> None:
...
class TestGetVersion(unittest.TestCase):
    ...
    def test_get_version_docker(self, mock_syscmd: MagicMock) -> None:
        ...
        product = DotMap(name='Docker', command='', regex=re.compile('Docker version (.+)'),
                        raw=False, args=[])
    ...
    def test_get_version_not_found(self, mock_syscmd: MagicMock) -> None:
        ...
        product = DotMap(name='MissingTool', command='missing', regex=re.compile('(.+)'),
                        raw=False, args=[])
    ...
    def test_get_version_cmd_error(self, mock_syscmd: MagicMock) -> None:
        ...
        product = DotMap(name='Tool', command='tool', regex=re.compile('(.+)'),
                        raw=False, args=[])
    ...
    def test_get_version_with_custom_args(self, mock_syscmd: MagicMock) -> None:
        ...
        product = DotMap(name='Helm', command='helm', regex=re.compile('Version (.+)'),
                        raw=False, args=['version'])

        version = get_version(product)

        self.assertIn('1.2.3', version)
        mock_syscmd.assert_called_once_with('helm', 'version', ignore_stderr=True,
                                           append_stderr=True)
    ...
    def test_get_version_no_regex_match(self, mock_syscmd: MagicMock) -> None:
        ...
        product = DotMap(name='Tool', command='tool', regex=re.compile('Version (.+)'),
                        raw=False, args=[])
    ...
    def test_get_version_gcloud(self, mock_syscmd: MagicMock) -> None:
        ...
        product = DotMap(name='Google Cloud SDK', command='gcloud',
                        regex=re.compile(r'Google Cloud SDK ([\d\.]+) '),
                        raw=False, args=[])
...
class TestGetHelmInfo(unittest.TestCase):
